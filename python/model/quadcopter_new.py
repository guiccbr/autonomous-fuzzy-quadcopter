...
class quadcopter():
	...
	def thrust(self,inputs, k):
		T = array([[0], [0], [k * np.sum(inputs)]])
		return T
		
	# Compute torques, given current inputs, length, drag coefficient, and thrust coefficient.
	def torques(self,inputs, L, b, k):
           #    tau = array([.0,.0,.0])
           #    tau[0] = L*k*(inputs[0]-inputs[1]-inputs[2]+inputs[3])
           #    tau[1] = L*k*(inputs[0]+inputs[1]-inputs[2]-inputs[3])
           #    tau[2] = b*(inputs[0]-inputs[1]+inputs[2]-inputs[3])
		tau = array([L*k*(inputs[1]**2-inputs[3]**2), L*k*(inputs[0]**2-inputs[2]**2), b*(inputs[0]**2-inputs[1]**2+inputs[2]**2-inputs[3]**2)])
		return tau
